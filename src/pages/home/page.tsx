...
import useRewards from "@/hooks/exportHooks";
import {useStakeContract} from "@/hooks/useContract";
import { Pid } from "../../utils";
import {useStakeContractWrite} from "@/utils/useStakeContractWagmi";
...
export default function Home() {
  ...
  // wagmi hooks
  const { useDepositETH, useClaim } = useStakeContractWrite();
  const { rewardsData, poolData, canClaim, refresh } = useRewards();
    ...
    const handleStake = async () => {
      ...
      // @ts-ignore
      if(!balance.formatted || parseFloat(balance.formatted) <  amount) {
      ...
      try {
        ...
        // const res = await waitForTransactionReceipt(data, { hash: tx });
          ...
          // await tx.wait()
        // wagmi hooks 版本
        const hash = await useDepositETH(
    ...
    const handleClaim = async () => {
      ...
      try {
        // viem 版本
        // const hash = await stakeContract.write.claim([Pid]);
        // ethers js sdk
        // @ts-ignore
        // const tx = await stakeContract.write.claim([Pid]);
        // await tx.wait();
        const hash = await useClaim(
          Pid
        );
        console.log("hash", hash)
        const res = await waitForTransactionReceipt(data!, { hash: hash });
        if(res.status === 'success') {
